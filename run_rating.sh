...
while sleep 2; do
    ...
    if [[ "$ELAPSED_TIME" -ge "$TIME_LIMIT" ]]; then
        echo "Time limit reached ($TIME_LIMIT seconds) - Killing process $PID2"
        kill -9 "$PID2"

        echo "Process ran for $ELAPSED_TIME seconds and was terminated" | tee -a "$LOG_FILE"
        break
    fi
done
